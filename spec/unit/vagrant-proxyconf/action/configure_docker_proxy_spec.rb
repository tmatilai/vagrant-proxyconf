...
def mock_write_docker_config(machine)
  ...
  allow(machine).to receive_message_chain(:communicate, :sudo).with("systemctl restart docker || systemctl restart docker.io || initctl restart docker || initctl restart docker.io || service docker restart || service docker.io restart || /etc/init.d/docker restart || /etc/init.d/docker.io restart || kill -HUP `pgrep -f 'docker'` || kill -HUP `pgrep -f 'docker.io'` ")
